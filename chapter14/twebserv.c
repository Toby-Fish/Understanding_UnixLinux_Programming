/* twebserv.c - a threaded minimal web server (vsrsion 0.2)
 ...
 * building: cc twebserv.c socklib.c -lpthraed -o twebserv
 */
...
extern int make_server_socket( int portnumber );
...
int main( int ac, char *av[] )
{
    ...
    while ( 1 ) {
        ...
        server_requests++;                          /* critical region */
 ...
 * skip over all request info util a CRNL is seen
 *****************************************************************************/
void skip_rest_of_header( FILE *fp )
{
    char buf[BUFSIZ];
...
/* handle built_in URLs here. Only one so far is "status" */
int built_in( char *arg, int fd )
{
    FILE    *fp;
